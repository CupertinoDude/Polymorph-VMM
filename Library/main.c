				 ...
				 struct Interface *Executive)
{
	...
	if((Base = IExec->OpenLibrary("commodities.library",50L)))
		Self->ICX = (APTR)IExec->GetInterface(Base,"main",1L,NULL);

	if((Base = IExec->OpenLibrary("datatypes.library",50L)))
		Self->IDataTypes = (APTR)IExec->GetInterface(Base,"main",1L,NULL);
	...
	IExec->InitSemaphore(&Self->ObjectCacheLock);
	IExec->NewList(&Self->ObjectCacheList);
	IExec->ObtainSemaphore(&Self->Lock);
	InitPolymorphDaemon(Self);
	InitPolymorphFilter(Self);
...
APTR LCALL_Expunge(struct LibraryManagerInterface *Iface)
{
	...
	if(Self->Library.lib_OpenCnt)
	{
		...
		if(Self->IDataTypes)
		{
			Base = Self->IDataTypes->Data.LibBase;
			IExec->DropInterface((APTR)Self->IDataTypes);
            IExec->CloseLibrary((APTR)Base);
		}
