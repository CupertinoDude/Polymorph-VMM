/*
**	Reiser3FileSystem - AmigaOS VectorPort FileSystem
*/
#include	"AmigaOS.h"
#include	"FileSystem.h"
#include	"FileSystemVectorPort.h"

#define	ID_REISER3FS_DISK	0x52465303
...
unsigned int32 Reiser3FileSystem( struct ExecBase *ExecBase)
{
	...
	struct fsApplication *fsApplication=NULL;
	...
	do{
		...
		if( !StartPacket )
		{
			...
			IExec->DebugPrintF("Reiser3FileSystem :: Workbench Launch applied\n",0);
		...
		if(!fsApplication)
		{
			...
			{
				IExec->DebugPrintF("[Reiser3FileSystem] Can't open V50 dos.library\n",0);
				break;
			}else{
			...
			{
				IExec->DebugPrintF("[Reiser3FileSystem] Can't open V50 utility.library\n",0);
				break;
			}else{
			...
			{
				IExec->DebugPrintF("[Reiser3FileSystem] Can't open DeviceIO.library\n",0);
				break;
			}else{
