...
static int32 do_setgroup(APTR dummy UNUSED,...) 
{
	return 0; /* Write me ! */ 
}
...
int32 FSSetGroup(struct FSVP *vp, int32 *res2, struct Lock *dirlock, 
                                 CONST_STRPTR objname, uint32 group) 
{
	...
	result = do_setgroup(gd,res2,lock,objname,group);
