...
static int32 do_create_softlink(APTR dummy UNUSED,...) 
{
	return 0; /* Write me ! */ 
}
...
int32 FSCreateSoftLink(struct FSVP *vp, int32 *res2, struct Lock *rel_dirlock, 
                       CONST_STRPTR link_name, CONST_STRPTR target_string)
{
	struct GlobalData *gd = vp->FSV.FSPrivate;
	struct ObjLock * lock = (struct ObjLock *)rel_dirlock;
	int32 result;

	IEXEC->ObtainSemaphore(gd->Sem);

	result = do_create_softlink(gd,res2,lock,link_name,target_string);
