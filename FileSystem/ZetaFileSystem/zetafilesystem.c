/*
**	ZetaFileSystem - AmigaOS VectorPort FileSystem
**
**	ZetaStorage and ZetaVolume are two independent vectorport entrypoints
**
**	ZetaStorage are RDSK partitions that are pooled together in a tree.
**		mirror or append rules are applied to build the tree.
**
**	ZetaVolume are User-Accessible File and Directory repositories
**		this uses a ZetaDataManager library overtop ZetaStorage
*/
#include	"AmigaOS.h"
#include	"FileSystem.h"
#include	"FileSystemVectorPort.h"

#define	ID_ZETA_STORAGE_DISK	0x5A465300
...
unsigned long fsObtainResources(struct fsApplication *fsAppl,struct DosPacket *startup)
{
	unsigned long rc=TRUE;

	do{
		/*
			FileSystem MemoryPool and Volume details.
		*/
	}while(0);

	if(!rc)
		fsReleaseResources(fsAppl);

	return(rc);
}

fsReleaseResources(struct fsApplication *fsAppl)
{
	return();
}

unsigned int32 ZetaFileSystem( struct ExecBase *ExecBase)
{
	unsigned int32 rc = RETURN_OK;
	...
	do{
		...
		if( !StartPacket )
		{
			IExec->DebugPrintF("ZetaFileSystem :: Workbench Launch applied\n",0);
		...
		if(!fsApplication)
		{
			...
			{
				IExec->DebugPrintF("[ZetaFileSystem] Can't open V50 dos.library\n",0);
				break;
			}else{
			...
			{
				IExec->DebugPrintF("[ZetaFileSystem] Can't open V50 utility.library\n",0);
				break;
			}else{
			...
			{
				IExec->DebugPrintF("[ZetaFileSystem] Can't open DeviceIO.library\n",0);
				break;
			}else{
